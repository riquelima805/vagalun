...
app.get('/api/billing/solana/:paymentId', authenticate, async (req, res) => {
  try {
    ...
    for (const sigInfo of sigs) {
      ...
      if (user) {
        // Excedente (se pagou um pouco a mais) fica de saldo pra cobrir os
        // próximos dias de cobrança automaticamente.
        user.balance = +((user.balance || 0) + (paidSol - payment.solanaAmount)).toFixed(9);
      }
    ...
    res.json({ ...payment, confirmed: false });
  } catch (error) {
