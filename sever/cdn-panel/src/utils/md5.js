// Implementação MD5 compacta (domínio público) — usada só para o preview
// client-side da assinatura no painel. A assinatura real de produção deve
// ser gerada no backend (media.js -> signStreamUrl), onde o secret vive.
export function md5(str) {
  ...
  function toHex(x) {
    ...
    for (let i = 0; i < x.length * 4; i++) {
      out += hc.charAt((x[i >> 2] >> ((i % 4) * 8 + 4)) & 0xf) + hc.charAt((x[i >> 2] >> ((i % 4) * 8)) & 0xf)
    }
    return out
  }
