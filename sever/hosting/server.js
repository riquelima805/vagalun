...
// Sem SMTP configurado (dev/local), a plataforma não trava: o link de
// verificação é só logado no console do servidor, pra dar pra testar sem
// precisar de um provedor de e-mail de verdade. Em produção, configure
// SMTP_HOST/PORT/USER/PASS (ex: Resend, SendGrid, Gmail app password) e o
// e-mail passa a ser enviado de verdade.
...
async function sendVerificationEmail(user) {
  const verifyUrl = `${CLIENT_URL}/app/verify-email?token=${user.verifyToken}`;
  if (!mailTransport) {
    console.log(`✉️  [dev] Link de verificação para ${user.email}: ${verifyUrl}`);
    return;
  }
  try {
    await mailTransport.sendMail({
      from: process.env.SMTP_FROM || 'no-reply@vagalun.shop',
      to: user.email,
...
// ============ COTAÇÃO R$ -> US$ (só pra exibição, não afeta cobrança) ============
// Busca numa API pública gratuita e guarda em cache por um tempo, com um
// valor fixo de fallback (env BRL_USD_FALLBACK_RATE) caso a API falhe ou o
// servidor não tenha saída pra internet nesse momento — nunca deixa a tela
// de billing quebrar por causa disso.
const BRL_USD_FALLBACK_RATE = Number(process.env.BRL_USD_FALLBACK_RATE || 0.18);
const BRL_USD_CACHE_MS = 60 * 60 * 1000; // 1 hora
let brlUsdCache = { rate: BRL_USD_FALLBACK_RATE, fetchedAt: 0 };

async function getBrlToUsdRate() {
  if (Date.now() - brlUsdCache.fetchedAt < BRL_USD_CACHE_MS) {
    return brlUsdCache.rate;
  }
  try {
    ...
    if (typeof rate === 'number' && rate > 0) {
      brlUsdCache = { rate, fetchedAt: Date.now() };
    }
  } catch (err) {
    console.error('[billing] falha ao buscar cotação BRL->USD, usando fallback:', err.message);
  }
  return brlUsdCache.rate;
}
...
async function runDailyBilling() {
  try {
    ...
    for (const user of users.values()) {
      ...
      // Cobrança é por CONTA (assinatura do plano), não por site — o plano já
      // limita quantos sites cabem via plan.maxSites. Antes multiplicava
      // perDay pela quantidade de sites publicados, o que fazia o saldo de
      // "1 mês" durar só balance / (perDay * nSites) dias em vez de 30.
      const costToday = perDay;
...
function loadDB() {
  ...
  try {
    ...
    (raw.users || []).forEach(([key, value]) => {
      // Migração: contas criadas antes da verificação de e-mail existir não
      // têm `emailVerified` gravado. Sem isso elas ficariam trancadas pra
      // sempre no login assim que essa feature entra em produção.
      if (value.emailVerified === undefined) value.emailVerified = true;
      users.set(key, value);
    });
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    const newUser = {
      ...
      verifyToken: REQUIRE_EMAIL_VERIFICATION ? verifyToken : null,
    ...
    if (REQUIRE_EMAIL_VERIFICATION) {
      ...
      // Não devolve token de sessão ainda: precisa confirmar o e-mail antes
      // de poder entrar (ver /api/auth/login abaixo).
      return res.json({
        userId,
...
app.get('/api/auth/verify-email', async (req, res) => {
  try {
    ...
    let matched = null;
    for (const u of users.values()) {
      if (u.verifyToken && u.verifyToken === token) { matched = u; break; }
    }
    if (!matched) return res.status(400).json({ error: 'Token inválido ou já usado' });

    matched.emailVerified = true;
    matched.verifyToken = null;
    saveDB();

    const sessionToken = generateToken(matched.userId);
    res.json({ success: true, token: sessionToken, userId: matched.userId });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
app.post('/api/auth/resend-verification', async (req, res) => {
  try {
    ...
    if (!user || user.emailVerified) {
      return res.json({ success: true, message: 'Se o e-mail existir e ainda não estiver confirmado, reenviamos o link.' });
    }
    user.verifyToken = crypto.randomBytes(32).toString('hex');
    user.verifyTokenCreatedAt = new Date();
    saveDB();
    await sendVerificationEmail(user);
    res.json({ success: true, message: 'Se o e-mail existir e ainda não estiver confirmado, reenviamos o link.' });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
  ...
  try {
    ...
    if (REQUIRE_EMAIL_VERIFICATION && !user.emailVerified) {
      return res.status(403).json({
        error: 'Confirme seu e-mail antes de entrar. Verifique sua caixa de entrada (e spam).',
...
app.get('/api/auth/me', authenticate, (req, res) => {
  ...
  const { password, verifyToken, ...safe } = user;
  res.json(safe);
});
...
// Apaga um site do usuário sob demanda (diferente do deleteSiteHard, que só
// roda automaticamente pra sites congelados há muito tempo sem saldo).
// Apaga: arquivos no(s) gateway node(s), a pasta local e o registro em `sites`.
app.delete('/api/sites/:siteId', authenticate, async (req, res) => {
  try {
    ...
    // Apaga cada arquivo publicado no(s) nó(s) do gateway (best-effort —
    // se um nó estiver offline não trava o resto da exclusão).
    const routes = siteRoutes(site);
    await Promise.allSettled(routes.map((r) => deleteFileRemote(r.fileId)));

    // Apaga a pasta local do site (fonte usada antes de publicar/preview).
    try {
      const siteDir = path.join(sitesDir, site.siteId);
      if (fs.existsSync(siteDir)) fs.rmSync(siteDir, { recursive: true, force: true });
    } catch (err) {
      console.error(`[sites] falha ao apagar pasta local do site ${site.siteId}:`, err.message);
    }

    sites.delete(site.siteId);
    saveDB();

    console.log(`🗑️  Site ${site.siteId} (${site.name}) apagado pelo usuário ${req.userId}.`);
...
app.get('/api/billing/plans', async (req, res) => {
  const usdRate = await getBrlToUsdRate();
  res.json({ plans: PLANS, brlToUsdRate: usdRate });
});
...
// Recarga de saldo "avulsa": mesmo fluxo do checkout de plano, mas sem
// vincular a nenhum plano — só soma R$ ao saldo (que a cobrança diária vai
// consumindo aos poucos). Existia só a opção de comprar um plano; sem isso
// quem já está no plano certo mas ficou com saldo baixo não tinha como
// recarregar sem "comprar" o plano de novo.
app.post('/api/billing/solana/recharge', authenticate, async (req, res) => {
  try {
    const amountBRL = Number(req.body?.amountBRL);
    if (!amountBRL || amountBRL <= 0) {
      return res.status(400).json({ error: 'Informe um valor de recarga válido.' });
    }
    if (amountBRL > 100000) {
      return res.status(400).json({ error: 'Valor de recarga muito alto.' });
    }
    ...
    const solanaPayUri = `solana:${SOLANA_TREASURY_WALLET}` +
      `?amount=${solanaAmount}&reference=${reference}` +
      `&label=Hospedagem&message=Recarga%20de%20saldo`;
    const qrCodeImage = await QRCode.toDataURL(solanaPayUri, { width: 300, margin: 1 });
    ...
    payments.set(paymentId, {
      ...
      amountBRL,
      solanaAmount,
      plan: null, // recarga não muda o plano, só o saldo
      reference,
      recipient: SOLANA_TREASURY_WALLET,
      status: 'pending',
      createdAt: new Date(),
    ...
    console.error('❌ Erro na recarga Solana:', error);
    res.status(500).json({ error: error.message });
  }
});
