...
async function serveFile(req, res, file, contentType, { etag, cacheControl } = {}) {
  ...
  // Item 15: se o navegador (ou uma CDN na frente) já tem esse ETag, nem
  // precisa buscar shard nenhum — 304 corta o trabalho todo antes de
  // começar, poupando gateway E os peers que guardam os shards de verdade.
  // Validação de ETag vale independente de Range (é assim que o HTTP
  // define condicional GET).
...
const server = http.createServer(async (req, res) => {
  ...
  if (!file.fileKeyB64) { sendJson(res, 403, { ok: false, error: 'arquivo não publicado para acesso via gateway' }); return; }
  // Diferente do /raw/:fileId: aqui o path pode apontar pra um fileId
  // DIFERENTE depois de um redeploy do site — não dá pra marcar
  // "immutable". Usa o fileId como ETag (ainda economiza banda via 304
  // quando não mudou) mas sem max-age longo, e com must-revalidate pra
  // forçar o navegador a sempre confirmar antes de reusar.
