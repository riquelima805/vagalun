...
async function fetchBlockPlaintext(file, block) {
  ...
  const placements = block.placements
    .map((p) => {
      ...
      if (!peer) return null;
      return { shardIndex: p.shardIndex, nodeId: p.nodeId, shardHash: p.shardHash, peer };
    })
    .filter(Boolean);
  ...
  const shards = await fetchShardsMultiSource(placements, file.k, async (p) => {
    ...
    if (!data) return null;
    // item 10: shard chegou, mas bate com o hash gravado no publish? Se
    // não bater, trata igual a "peer não respondeu" — o multiSource já
    // sabe pular pro próximo placement até juntar k bons. Diferença de
    // um fetch que falhou de verdade: aqui SABEMOS que o peer respondeu
    // um shard, só que ele tá errado — vale logar quem foi, é sinal pro
    // futuro self-healing (item 11) considerar esse peer suspeito.
