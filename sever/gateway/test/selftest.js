...
// ---------- 4b) rate limit (item 14) ----------
console.log('\n[4b] rate limit (sliding window em memória)');
{
  check('permite até o limite, bloqueia no seguinte', () => {
    const key = `test-${Math.random()}`;
    for (let i = 0; i < 5; i++) {
      assert.strictEqual(rateLimited(key, 5, 10_000), false, `chamada ${i + 1} deveria passar`);
    }
    assert.strictEqual(rateLimited(key, 5, 10_000), true, '6ª chamada deveria estourar o limite');
  });
  check('janelas diferentes (chaves diferentes) não se misturam', () => {
    const keyA = `test-a-${Math.random()}`;
    const keyB = `test-b-${Math.random()}`;
    for (let i = 0; i < 5; i++) rateLimited(keyA, 5, 10_000);
    assert.strictEqual(rateLimited(keyA, 5, 10_000), true, 'A deveria estar bloqueado');
    assert.strictEqual(rateLimited(keyB, 5, 10_000), false, 'B não deveria ser afetado pelo limite de A');
  });
}
...
async function e2e() {
  ...
  while (offset < originalBuf.length) {
    ...
    for (let s = 0; s < k + m; s++) {
      ...
      placements.push({ shardIndex: s, nodeId: `node-${s}`, shardHash: hashShard(encoded.shards[s]) });
    }
  ...
  await checkAsync('item 10: shard corrompido no peer é detectado (hash não bate) e ignorado, não contamina o resultado', async () => {
    const file = registry.getFile(fileId);
    const block = file.blocks[0];
    content.blockCache.clear();

    // corrompe o shard 0 direto no "storage" do peer fake (simula bit
    // flip / transferência incompleta) — sem tocar no manifesto, que
    // continua com o shardHash correto (é o que faz a verificação falhar).
    const shardKey = content.shardKeyFor(fileId, block.blockIndex, 0);
    const good = shardsByKey.get(shardKey);
    const corrupted = Buffer.from(good);
    corrupted[0] ^= 0xff; // flipa o primeiro byte
    shardsByKey.set(shardKey, corrupted);

    try {
      const plaintext = await content.fetchBlockPlaintext(file, block);
      // com k+m nós e só 1 corrompido, ainda sobram nós suficientes pra
      // juntar k shards bons — o resultado tem que ser IDÊNTICO ao
      // original, provando que o shard ruim foi descartado, não usado.
      assert.ok(Buffer.compare(plaintext, originalBuf.subarray(0, block.plainLength)) === 0);
    } finally {
      shardsByKey.set(shardKey, good); // restaura pros próximos testes
    }
  });

  check('item 10: registerFile rejeita manifesto sem shardHash em algum placement', () => {
    assert.throws(() => {
      registry.registerFile({
        fileId: 'sem-hash-1',
        fileName: 'x',
        ...
        blockSize,
        originalLength: 10,
        blocks: [{ blockIndex: 0, plainLength: 10, shardSize: 10, iv: 'x', authTag: 'y', placements: [{ shardIndex: 0, nodeId: 'node-0' }] }],
        fileKeyB64: fileKey.toString('base64'),
      });
    }, /shardHash/);
  });
  ...
  await checkAsync('GET /raw/:fileId devolve ETag e Cache-Control imutável', async () => {
    const resp = await fetch(`http://127.0.0.1:${gwPort}/raw/${fileId}`);
    assert.strictEqual(resp.headers.get('etag'), `"${fileId}"`);
    assert.ok(resp.headers.get('cache-control')?.includes('immutable'), 'cache-control deveria ter immutable');
  });

  await checkAsync('GET /raw/:fileId com If-None-Match igual -> 304 sem corpo', async () => {
    const resp = await fetch(`http://127.0.0.1:${gwPort}/raw/${fileId}`, { headers: { 'If-None-Match': `"${fileId}"` } });
    assert.strictEqual(resp.status, 304);
    const body = await resp.arrayBuffer();
    assert.strictEqual(body.byteLength, 0, '304 não deveria ter corpo');
  });

  await checkAsync('GET /raw/:fileId com If-None-Match diferente -> 200 normal', async () => {
    const resp = await fetch(`http://127.0.0.1:${gwPort}/raw/${fileId}`, { headers: { 'If-None-Match': '"outro-hash-qualquer"' } });
  ...
  await checkAsync('item 14: rate limit por IP+fileId estoura em 429 depois do limite', async () => {
    // limite é 120/60s por IP+fileId — usa If-None-Match variando pra
    // cada request não bater no early-return de 304 (queremos forçar o
    // caminho normal e ver o 429 aparecer)
    let got429 = false;
    for (let i = 0; i < 130 && !got429; i++) {
      const resp = await fetch(`http://127.0.0.1:${gwPort}/raw/${fileId}`, { headers: { 'If-None-Match': `"rl-test-${i}"` } });
      if (resp.status === 429) got429 = true;
    }
    assert.ok(got429, 'deveria ter recebido 429 em algum momento dentro de 130 requisições');
  });
