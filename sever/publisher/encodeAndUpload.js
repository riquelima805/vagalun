...
const reedSolomon = require('../gateway/reedSolomon');
const shardTransport = require('../gateway/shardTransport');
const { hashShard } = require('../gateway/shardHash');
...
async function encodeAndUpload(buffer, { fileId, k, m, nodes, blockSize = DEFAULT_BLOCK_SIZE, putShardFn = defaultPutShard }) {
  ...
  while (offset < buffer.length || (buffer.length === 0 && blockIndex === 0)) {
    ...
    for (let s = 0; s < n; s++) {
      ...
      const node = nodes[s];
      // hash calculado ANTES do upload, sobre os bytes que vamos mandar —
      // é o que garante que qualquer corrupção depois desse ponto (na
      // transferência, ou já no storage do peer) vai ser pega depois.
      const shardHash = hashShard(encoded.shards[s]);
      ...
      placements.push({ shardIndex: s, nodeId: node.nodeId, shardHash });
    }
